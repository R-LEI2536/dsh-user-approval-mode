...
- **Field descriptions are now inline** — each field's description is rendered on its own row below the field (full container width), so it doesn't get squeezed into the narrow label column. Same `desc.*` locale keys, no more `?`-icon tooltips.
